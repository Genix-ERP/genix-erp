...
export default function AgedReceivables() {
  ...
  const loadData = async (date) => {
    setIsLoading(true);
    try {
      const result = await financeService.getAgingReceivables({ as_of_date: date });
      setData(result);
    } catch (err) {
      console.error('Failed to load aging receivables:', err);
      setData(null);
    } finally {
      setIsLoading(false);
    }
        ...
        <CardContent className="p-0">
          {isLoading ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/80">
                    ...
                    <SortHeader field="name">{t('partner') || 'Partner'}</SortHeader>
            ...
            <div className="flex flex-col items-center justify-center h-48 text-slate-400">
              ...
              <p>{t('no_outstanding_receivables') || 'No outstanding receivables'}</p>
            </div>
          )}
        </CardContent>
      </Card>
